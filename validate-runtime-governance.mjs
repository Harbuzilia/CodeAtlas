...
const pkgOpencodePath = path.join(root, '.opencode', 'package.json');
if (!fs.existsSync(pkgOpencodePath)) {
  // Fresh clone / CI: the file is gitignored on purpose and generated by sync:local.
  fail('Missing .opencode/package.json — run npm run sync:local (validate:all does this first)');
} else {
  const pkgOpencode = JSON.parse(fs.readFileSync(pkgOpencodePath, 'utf8'));
