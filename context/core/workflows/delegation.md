...
14. **PROGRESS-LOG**: Для цепочки из 2+ делегирований веди `.opencode/progress.md` (`HH:MM → agent: задача` перед task(), `HH:MM ✓ agent: итог` после возврата) — это единственный живой канал «что происходит сейчас» для пользователя. Записи — это tool-вызовы (edit/write), не текст в чат, правило 13 не нарушают.
15. **FINDINGS-DURABLE**: После возврата discovery-агента (contextscout/externalscout) допиши 3-7 строк его ключевых находок в `.opencode/findings.md` и передавай релевантные findings следующему субагенту в prompt — они переживают compaction. Скауты сами ничего не пишут (read-only).
