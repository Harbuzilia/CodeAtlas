...
Для production/runtime инцидентов ОБЯЗАТЕЛЬНО загружай `skill/tools/incident-response.md`
перед диагностикой и follow this playbook (triage -> containment -> root cause -> fix -> validate).
</critical_instruction>
...
<integration>
  Вызывается из:
  - `subagents/code/coder-agent` — после write/edit при ошибке build
  ...
  - Рекомендовать `subagents/code/tester` для тестов
  - Рекомендовать `subagents/code/reviewer` для review
</integration>
