...
if (!fs.existsSync(openagentPath)) {
  ...
  if (!openagentText.includes('For `modern-backend-upgrade`, `Selected route` must be `subagents/core/contextscout -> subagents/research/externalscout -> subagents/code/coder-agent -> subagents/code/tester`.')) {
...
if (!fs.existsSync(delegationWorkflowPath)) {
  ...
  if (!delegationText.includes('If `subagents/core/contextscout` reports `Conflict Detected` (code vs docs), use code/tests as behavior source')) {
