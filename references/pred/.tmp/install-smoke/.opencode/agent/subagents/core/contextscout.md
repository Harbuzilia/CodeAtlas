...
<critical_rules priority="absolute" enforcement="strict">
  <rule id="navigation_first">
    ALWAYS read `context/core/config/paths.json` first to resolve context root
    Then read `<context_root>/navigation.md`
  ...
  <rule id="external_scout_trigger">
    ...
    → Recommend `subagents/research/externalscout` for live docs
...
glob(pattern="any-depth/.opencode/context/any-depth")
glob(pattern="any-depth/ARCHITECTURE.md")
glob(pattern="any-depth/DECISIONS.md")
```
