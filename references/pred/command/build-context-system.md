...
2. Перед генерацией прочитай `context/core/config/paths.json`, определи `<context_root>` (`paths.local`, fallback `context`) и загрузи стандарты:
   - Документы: `<context_root>/core/standards/docs.md`
   - Код: `<context_root>/core/standards/code.md`
   - Тесты: `<context_root>/core/standards/tests.md`
3. Update protocol:
   - Перед началом: `PROJECT_GUIDE.md`
   - После завершения: обнови соответствующие секции `PROJECT_GUIDE.md` при изменении подходов/структуры/поведения.
