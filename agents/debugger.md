...
<hard_rules>
  ...
  <rule>[W1] Если Попытка 1 неудачна — останови догадки. Найди точный текст ошибки и версию библиотеки встроенным поиском `google_search` (StackOverflow/GitHub) или через `webfetch` по прямой ссылке; для версионной документации библиотеки предпочтительнее делегировать `externalscout`. Затем примени найденное решение во 2-й попытке.</rule>
...
<workflow>
  ...
  <stage id="5" name="Validate">
    ...
    2. Если успех → готово
    3. Если та же ошибка после Попытки 1 → поискать через встроенный `google_search` (или `webfetch` по прямой ссылке): "<текст ошибки> <версия библиотеки>"
